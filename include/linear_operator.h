                            ...
                            const MPI_Comm &mpi_communicator)
{
  ...
  return_op.vmult = [&cm, &locally_owned_dofs,&locally_relevant_dofs, &mpi_communicator](Range &v, const Domain &u)
  {
    Range loc_src(  locally_owned_dofs,
                    locally_relevant_dofs,
                    mpi_communicator);
    loc_src.reinit(u,false,true);
    // Vector<double> loc_src(u);
            ...
            for (types::global_dof_index j=0; j < entries->size(); ++j)
              {
                ...
                v(i) +=  loc_src(pos) * (*entries)[j].second;
              }
          }
        else
          v(i)+=loc_src(i);
  ...
  return_op.Tvmult = [&cm, &locally_owned_dofs,&locally_relevant_dofs, &mpi_communicator](Range &v, const Domain &u)
  {
    for (auto i : u.locally_owned_elements())
      {
        if (cm.is_constrained(i))
          {
            if (v.locally_owned_elements().is_element(i))
              v(i)-=v(i);
        ...
        else if (v.locally_owned_elements().is_element(i))
          {
            v(i)-=v(i);
            v(i)+=u(i);
          }
...
constraints_linear_operator(const ConstraintMatrix &cm, const Matrix &m)
{
  ...
  {
    // v.locally_owned_elements() == complete_index_set (v.size())
    for (auto i : v.locally_owned_elements())
      {
        if (cm.is_constrained(i))
          {
...
constrained_linear_operator(const ConstraintMatrix &cm, const Matrix &m,
                            ...
                            const MPI_Comm &mpi_communicator)
{
  auto C  = constraints_linear_operator<Range, Domain, Matrix>(cm, m, locally_owned_dofs, locally_relevant_dofs, mpi_communicator);
  auto Ct = transpose_operator<Domain, Range>(C);
  auto A  = linear_operator<Range, Domain>(m);
                ...
                const MPI_Comm &mpi_communicator)
{
  ...
  return_comp.apply = [&cm, &m, &rhs, &locally_owned_dofs, &locally_relevant_dofs, &mpi_communicator](Range &v)
  {
    const auto M = linear_operator<Range, Domain>(m);
    const auto C  = constraints_linear_operator<Range, Domain, Matrix>(cm, m, locally_owned_dofs, locally_relevant_dofs, mpi_communicator);
    const auto Ct = transpose_operator<Domain, Range>(C);
